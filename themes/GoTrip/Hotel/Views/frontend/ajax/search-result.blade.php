        ...
        @foreach($rows as $row)
            ...
            @if($layout == 'grid')
                <div class="col-lg-4 col-sm-6">
                    @include('Hotel::frontend.layouts.search.loop-grid',['wrap_class'=>'item-loop-wrap'])
                </div>
            @else
                <div class="col-12">
                    @include('Hotel::frontend.layouts.search.loop-list',['wrap_class'=>'item-loop-wrap inner-loop-wrap'])
                </div>
            @endif
        @endforeach
    @else
